...
export function validateComplianceInput(
  ...
  if (
    !TIPOS_VERIFICACAO.includes(
      tipoVerificacao as (typeof TIPOS_VERIFICACAO)[number],
    )
  ) {
  ...
  if (
    !documentoTexto ||
    typeof documentoTexto !== "string" ||
    documentoTexto.length < 20
  ) {
    throw new ValidationError(
