 ...
 * ```
 */
export function validateGestaoPrazosInput(
  ...
  if (
    !TIPOS_PROCESSO_VALIDOS.includes(
      tipoProcesso as (typeof TIPOS_PROCESSO_VALIDOS)[number],
    )
  ) {
    throw new ValidationError(
  ...
  if (
    considerarFeriados !== undefined &&
    typeof considerarFeriados !== "boolean"
  ) {
    throw new ValidationError(
  ...
  if (
    considerarRecessoForense !== undefined &&
    typeof considerarRecessoForense !== "boolean"
  ) {
    throw new ValidationError(
