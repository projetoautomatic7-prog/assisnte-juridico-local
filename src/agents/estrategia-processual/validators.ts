...
export function validateEstrategiaProcessualInput(data: Record<string, unknown>): EstrategiaProcessualInput {
  const tipoCaso = (data.tipoCaso as string) || "trabalhista";
  if (!TIPOS_CASO.includes(tipoCaso as typeof TIPOS_CASO[number])) {
    throw new ValidationError(`Campo 'tipoCaso' deve ser: ${TIPOS_CASO.join(", ")}`, "tipoCaso", tipoCaso);
  }

  const faseProcessual = (data.faseProcessual as string) || "inicial";
  if (!FASES.includes(faseProcessual as typeof FASES[number])) {
    throw new ValidationError(`Campo 'faseProcessual' deve ser: ${FASES.join(", ")}`, "faseProcessual", faseProcessual);
  }
