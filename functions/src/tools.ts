  ...
  async (input) => {
    ...
    try {
      ...
      try {
        ...
        type PdfParseResult = { text: string; metadata?: unknown; numpages?: number };
        type PdfParseFn = (input: Buffer) => Promise<PdfParseResult>;

        const pdfParseModule = (await import("pdf-parse")) as { default?: PdfParseFn };
        const pdfParseFn: PdfParseFn =
          pdfParseModule.default ?? (pdfParseModule as unknown as PdfParseFn);
      ...
      logger.info("[PDF] Processamento de PDF concluído", {
        ...
        chunksIndexed: (result as { chunksIndexed?: number }).chunksIndexed,
      ...
      return {
        ...
        chunksIndexed: (result as { chunksIndexed?: number }).chunksIndexed ?? 0,
      };
    } catch (error) {
      const totalDuration = Date.now() - startTime;
...
export const AGENT_TOOLS = ALL_TOOLS.map(toAgentTool);
